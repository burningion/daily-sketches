...
fileLocation = '50slowin/'
numFiles = len(list(glob.glob('50slowin/*.png')))
...
for i in range(1, numFiles - 2):
    ...
    if r['rois'].shape[0] >= 1:
        ...
        paster.save('50sloframes/%05d.png' % counter)
        imageFrames.append(paster)
    else:
        frame = cv2.cvtColor(frame.astype('uint8'), cv2.COLOR_BGR2RGBA)
        frame = Image.fromarray(frame)
        paster.save('50sloframes/%05d.png' % counter)
...
for i in range(1, numFiles - 1):
    filename = fileLocation + ('%05d.png' % i)
    print("doing out frame %s" % filename)
    frame = Image.open(filename).convert('RGBA')
    if i > 5:
        for j in range(i % 60):
            frame.paste(imageFrames[i - j], (0,0), imageFrames[i - j])
            frame.paste(imageFrames[i + j], (0,0), imageFrames[i + j])
    frame = frame.convert('RGB')
    frame.save('50sloout/%05d.jpg' % counter)
    counter += 1
