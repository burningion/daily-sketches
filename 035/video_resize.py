...
numFiles = len(list(glob.glob('kirkbowl/*.png')))

imageWidth, imageHeight = 1920, 1080

for i in range(1, numFiles - 1):
    filename = 'kirkbowl/%05d.png' % i
    ...
    if r['rois'].shape[0] >= 1:
        ...
        for j in range(5):
            pasterB = paster.resize((1920 + 100 * abs(j - 4), 1080 + 100 * abs(j - 4)))
            maskB = mask.resize((1920 + 100 * abs(j - 4), 1080 + 100 * abs(j - 4)))
            frame.paste(pasterB, (-100 * abs(j - 4), -100 * abs(j - 4)), mask=maskB)
    else:
        frame = cv2.cvtColor(frame.astype('uint8'), cv2.COLOR_BGR2RGBA)
        frame = Image.fromarray(frame)
    
    fileout = 'bowlseq/%05d.jpg' % i
    print(fileout)
