...
fileLocation = 'tailbodyvarial/'
outLocation = 'tailbodyvarialout/'

if not os.path.exists(outLocation):
    os.makedirs(outLocation)

numFiles = len(list(glob.glob(fileLocation + '*.png')))
...
for i in range(1, numFiles - 1):
    ...
    if r['rois'].shape[0] >= 1:
        for b in range(r['rois'].shape[0]):
            if r['class_ids'][b] == class_names.index('skateboard') or r['class_ids'][b] == class_names.index('person'):
                print('board or human detected')
                mask = r['masks'][:,:,b] * 255
                masky += mask.astype(np.uint8)
        # blur and dilate the mask so we cover most of person
        masky = cv2.blur(masky, (1,1), cv2.BORDER_TRANSPARENT)
        masky = cv2.dilate(masky, np.ones((3,3),np.uint8), iterations=2)
        ...
        for f in range(20):
            maskR = Image.fromarray(np.roll(mask, (f + 1) * 15, axis=1))
            maskL = Image.fromarray(np.roll(mask, (f + 1) * -15, axis=1))
            paster.paste(Image.fromarray(np.roll(frame, (f + 1) * 25, axis=1)), (0,0), mask=maskR)
            paster.paste(Image.fromarray(np.roll(frame, (f + 1) * -25, axis=1)), (0,0), mask=maskL)

        # paster.paste(frame, (0,0), mask=mask)
    ...
    fileout = f"{outLocation}{counter:05d}.jpg"
    print(fileout)
