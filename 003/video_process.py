...
for i in range(1, 1030):
    filename = 'skateseq/%05d.png' % i
    spacename = 'fallsseq/%05d.png' % (i % 167 + 1)
    ...
    if r['rois'].shape[0] >= 1:
        ...
        new_mask = cv2.cvtColor(new_mask, cv2.COLOR_BGR2BGRA)
        frame = blend_modes.hard_light(frame.astype(float), new_mask.astype(float), .8)
    fileout = 'fallsseqout/%05d.jpg' % i
    print(fileout)
