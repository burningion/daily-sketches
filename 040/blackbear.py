...
fileLocation = 'bbimagesin/'
numFiles = len(list(glob.glob('bbimagesin/*.png')))
...
for i in range(1, numFiles - 1):
    ...
    if r['rois'].shape[0] >= 1 and counter > 60:
        ...
        for j in reversed(range(boards)):
            if j == 0:
                continue
            currentDist = math.sin(counter * .05) * boardDist
            currentDistPaste = currentDist - math.sin(counter * .1) * 150
            frame.paste(pasterB, (int(-currentDistPaste * j), int(0 - currentDist / 2)), mask=maskB)
            frame.paste(ImageOps.mirror(pasterB), (int(currentDistPaste * j), int(0 - currentDist / 2)),
                        mask=ImageOps.mirror(maskB))
    ...
    fileout = 'bbimageout/%05d.jpg' % counter
    print(fileout)
